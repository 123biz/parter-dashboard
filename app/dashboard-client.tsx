...
export default function DashboardClient({ partners }: { partners: Partner[] }) {
  ...
  const maxRevenue = getPartnerRevenue(top5[0])
  ...
  return (
    <div className="min-h-screen bg-[#f5f6fa] text-gray-900">
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex items-center justify-between shadow-sm">
        ...
        <span className="text-xs text-gray-400">Aantigravity Inc.</span>
      </header>
      ...
      <main className="px-8 py-6 space-y-6 max-w-[1600px] mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard
            title="총 거래처"
            value={`${partners.length}개`}
            sub="등록된 전체 거래처 수"
            accent="blue"
          />
          <StatCard
            title="이번 달 총 매출"
            value={formatRevenue(getTotalRevenue(partners))}
            sub="전 대리점 합산"
            accent="green"
          />
          <StatCard
            title="정상 거래처"
            value={`${getNormalCount(partners)}개`}
            sub={`전체의 ${Math.round((getNormalCount(partners) / partners.length) * 100)}%`}
            accent="green"
          />
          <StatCard
            title="경고 거래처"
            value={`${getWarningCount(partners)}개`}
            sub="점검이 필요한 거래처"
            accent="yellow"
          />
        </div>
            ...
            <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <div className="flex items-center gap-4 text-xs text-gray-400">
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-emerald-500" />정상
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-yellow-500" />경고
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2 h-2 rounded-full bg-blue-500" />선택됨
                </span>
                <span className="text-gray-300">|</span>
                <span className="text-gray-400">버블 크기 = 매출 비례</span>
              </div>
            </div>
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      selectedId === p.id ? 'bg-blue-50 ring-1 ring-blue-300' : 'hover:bg-gray-50',
                    ].join(' ')}
                  >
